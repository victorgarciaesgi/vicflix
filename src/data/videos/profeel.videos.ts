import { ProjectVideo } from '@models';

const projectId = 'LAtFkd';

export const profeelVideos: ProjectVideo[] = [
  {
    id: 'INiu876',
    title: 'Inscription',
    description: "Démo de l'inscription à Profeel",
    duration: 79,
    episode: 1,
    season: 1,
    projectId,
    videoUrl:
      'https://firebasestorage.googleapis.com/v0/b/vicflix-videos.appspot.com/o/Profeel%2FInscription.mp4?alt=media&token=f1872cfc-5a3a-4c00-8780-79e8de6a69ba',
  },
  {
    id: 'NLz6HLH',
    title: 'Front office client',
    description: "Petit tour de l'application client",
    duration: 41,
    episode: 2,
    season: 1,
    projectId,
    videoUrl:
      'https://firebasestorage.googleapis.com/v0/b/vicflix-videos.appspot.com/o/Profeel%2FPresentation%20FO.mp4?alt=media&token=e62eb4bb-6ea2-44fb-9205-195a90e9bed9',
  },
  {
    id: 'MOzlefnln',
    title: 'Création de chantier',
    description: 'Démonstration du formulaire de création de chantier',
    duration: 130,
    episode: 3,
    season: 1,
    projectId,
    videoUrl:
      'https://firebasestorage.googleapis.com/v0/b/vicflix-videos.appspot.com/o/Profeel%2FCreation%20chantier.mp4?alt=media&token=5d0eac88-d823-41d4-8524-9991861bfe2b',
  },
  {
    id: 'lzhdefeiz',
    title: 'Information Chantier',
    description: 'Regardons le chantier crée!',
    duration: 71,
    episode: 4,
    season: 1,
    projectId,
    videoUrl:
      'https://firebasestorage.googleapis.com/v0/b/vicflix-videos.appspot.com/o/Profeel%2FInformation%20Chantier.mp4?alt=media&token=4b412ea3-21a6-4c24-8344-df80703f4ffc',
  },
  {
    id: 'BHBZhlzei',
    title: 'Autocontroles',
    description: `Nous pouvons aussi accéder aux autocontrôles d'un chantier`,
    duration: 87,
    episode: 5,
    season: 1,
    projectId,
    videoUrl:
      'https://firebasestorage.googleapis.com/v0/b/vicflix-videos.appspot.com/o/Profeel%2FAutocontroles.mp4?alt=media&token=1b10e778-f91c-4f28-b65b-85ef3029eac8',
  },
  {
    id: 'psoknzuhf',
    title: 'Création de chantier',
    description: 'Mais aussi des réceptions!',
    duration: 129,
    episode: 6,
    season: 1,
    projectId,
    videoUrl:
      'https://firebasestorage.googleapis.com/v0/b/vicflix-videos.appspot.com/o/Profeel%2FReceptions.mp4?alt=media&token=6cb250b8-cc40-4814-a883-37475fa3c193',
  },
];
