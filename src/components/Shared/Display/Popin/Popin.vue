  ...
  <Fragment @click.native="handleClick">
    ...
    <Portal v-if="rendered" to="Popup-Outlet">
      ...
      >
        ...
        <div
          :class="{ rounded, 'shadow-bottom': isBottom, 'shadow-top': isTop }"
          class="bg-bg1 border-bg5 max-h-inh flex overflow-x-hidden overflow-y-auto border"
        >
          <slot :alignement="alignement" name="popup" />
        </div>
      </div>
    </Portal>
...
<script lang="ts">
import { Vue, Prop, Component, Watch, PropSync, Emit } from 'nuxt-property-decorator';
import { EventBus, Events } from '@services';
import debounce from 'lodash/debounce';
import { Fragment } from 'vue-fragment';
import { nanoid } from 'nanoid';
import SvgIcon from '../SvgIcon.vue';
import { getPopupComputedOutputMesures, getScrollParent, acceptedClonedStyles } from './utils';
...
export default class Popin extends Vue {
  ...
  @Prop() alignement?: PopupAlignement;
  ...
  get getArrowSrc(): string {
    return 'icons/popup/arrow_bottom.svg';
